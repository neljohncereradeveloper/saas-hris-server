...
@Injectable()
export class ReligionRepositoryImpl
  implements ReligionRepository<EntityManager>
{
  constructor(
    @InjectRepository(ReligionEntity)
    private readonly repository: Repository<ReligionEntity>,
  ) {}

  async create(religion: Religion, manager: EntityManager): Promise<Religion> {
    const query = `
      INSERT INTO religion (desc1) 
      VALUES ($1) 
      RETURNING id, desc1, isactive
    `;
    const result = await manager.query(query, [religion.desc1]);
  ...
  async findById(id: number, manager: EntityManager): Promise<Religion | null> {
    const query = 'SELECT id, desc1, isactive FROM religion WHERE id = $1';
  ...
  async findPaginatedList(
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM religion ${whereClause}`;
  ...
  async findByDescription(
    ...
    const query =
      'SELECT id, desc1, isactive FROM religion WHERE desc1 = $1 AND isactive = true';
  ...
  async retrieveForCombobox(): Promise<Religion[]> {
    const query =
      'SELECT id, desc1, isactive FROM religion WHERE isactive = true ORDER BY desc1 ASC';
