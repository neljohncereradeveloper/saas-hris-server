...
import { TrainingCertEntity } from '../entities/training-cert.entity';
...
@Injectable()
export class TrainingCertRepositoryImpl
  implements TrainingCertRepository<EntityManager>
{
  constructor(
    @InjectRepository(TrainingCertEntity)
    private readonly repository: Repository<TrainingCertEntity>,
  ) {}
  ...
  async create(
    ...
    const query = `
      INSERT INTO training_cert (desc1) 
      VALUES ($1) 
      RETURNING id, desc1, isactive
    `;
    const result = await manager.query(query, [trainingCert.desc1]);

    const entity = result[0];
    return new TrainingCert({
      id: entity.id,
      desc1: entity.desc1,
  ...
  async findById(
    ...
    const query = 'SELECT id, desc1, isactive FROM training_cert WHERE id = $1';
  ...
  async findPaginatedList(
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM training_cert ${whereClause}`;
  ...
  async findByDescription(
    ...
    const query =
      'SELECT id, desc1, isactive FROM training_cert WHERE desc1 = $1 AND isactive = true';
  ...
  async retrieveForCombobox(): Promise<TrainingCert[]> {
    const query =
      'SELECT id, desc1, isactive FROM training_cert WHERE isactive = true ORDER BY desc1 ASC';
