...
export class CityRepositoryImpl implements CityRepository<EntityManager> {
  ...
  async create(city: City, manager: EntityManager): Promise<City> {
    const query = `
      INSERT INTO city (desc1) 
      VALUES ($1) 
      RETURNING *
    `;
    const result = await manager.query(query, [city.desc1]);
  ...
  async findById(id: number, manager: EntityManager): Promise<City | null> {
    const query = 'SELECT id, desc1, isactive FROM city WHERE id = $1';
  ...
  async findPaginatedList(
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM city ${whereClause}`;
  ...
  async findByDescription(
    ...
    const query =
      'SELECT id, desc1, isactive FROM city WHERE desc1 = $1 AND isactive = true';
  ...
  async retrieveForCombobox(): Promise<City[]> {
    const query =
      'SELECT id, desc1, isactive FROM city WHERE isactive = true ORDER BY desc1 ASC';
