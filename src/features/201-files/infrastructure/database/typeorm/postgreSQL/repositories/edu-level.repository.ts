...
import { EduLevelEntity } from '../entities/edu-level.entity';
...
@Injectable()
export class EduLevelRepositoryImpl
  implements EduLevelRepository<EntityManager>
{
  constructor(
    @InjectRepository(EduLevelEntity)
    private readonly repository: Repository<EduLevelEntity>,
  ) {}
  ...
  async create(
    ...
    const query = `
      INSERT INTO edu_level (desc1) 
      VALUES ($1) 
      RETURNING *
    `;
    const result = await manager.query(query, [educationLevel.desc1]);
  ...
  async findById(id: number, manager: EntityManager): Promise<EduLevel | null> {
    const query = 'SELECT id, desc1, isactive FROM edu_level WHERE id = $1';
  ...
  async findPaginatedList(
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM edu_level ${whereClause}`;
  ...
  async findByDescription(
    ...
    const query =
      'SELECT id, desc1, isactive FROM edu_level WHERE desc1 = $1 AND isactive = true';
  ...
  async retrieveForCombobox(): Promise<EduLevel[]> {
    const query =
      'SELECT id, desc1, isactive FROM edu_level WHERE isactive = true ORDER BY desc1 ASC';
