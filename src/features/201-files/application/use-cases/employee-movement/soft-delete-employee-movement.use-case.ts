...
@Injectable()
export class SoftDeleteEmployeeMovementUseCase {
  constructor(
  ...
  async execute(
    ...
    return this.transactionHelper.executeTransaction(
      CONSTANTS_LOG_ACTION.SOFT_DELETE_EMPLOYEE_MOVEMENT,
      async (manager) => {
        ...
        const errorHandlerOptions = this.errorHandler.createOptions(
          ...
          { id, isActive },
          requestInfo,
          `Employee movement has been ${action}d`,
          `Failed to ${action} employee movement with ID: ${id}`,
        );
        ...
        return this.errorHandler.executeWithErrorHandling(
          async () => {
            ...
            const softDeleteSuccessfull =
              await this.employeeMovementRepository.softDelete(
                id,
                isActive,
                manager,
              );
            if (!softDeleteSuccessfull) {
              throw new SomethinWentWrongException(
                'Employee movement soft delete failed',
              );
            }
