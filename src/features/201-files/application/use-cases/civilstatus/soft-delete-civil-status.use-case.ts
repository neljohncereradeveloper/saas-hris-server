...
@Injectable()
export class SoftDeleteCivilStatusUseCase {
  constructor(
    ...
    return this.transactionHelper.executeTransaction(
      ...
      async (manager) => {
        ...
        const errorHandlerOptions = this.errorHandler.createOptions(
          ...
          { id, isActive },
          requestInfo,
          `Civil status has been ${action}d`,
          `Failed to ${action} civil status with ID: ${id}`,
        );
        ...
        return this.errorHandler.executeWithErrorHandling(
          async () => {
            ...
            const softDeleteSuccessfull =
              await this.civilStatusRepository.softDelete(
                id,
                isActive,
                manager,
              );
            if (!softDeleteSuccessfull) {
              throw new SomethinWentWrongException(
                'CivilStatus soft delete failed',
              );
            }
