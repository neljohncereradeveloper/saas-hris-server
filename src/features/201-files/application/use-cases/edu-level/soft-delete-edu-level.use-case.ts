...
@Injectable()
export class SoftDeleteEduLevelUseCase {
  constructor(
    ...
    return this.transactionHelper.executeTransaction(
      ...
      async (manager) => {
        ...
        const errorHandlerOptions = this.errorHandler.createOptions(
          ...
          { id, isActive },
          requestInfo,
          `Education level has been ${action}d`,
          `Failed to ${action} education level with ID: ${id}`,
        );
        ...
        return this.errorHandler.executeWithErrorHandling(
          async () => {
            ...
            // Soft delete the edulevel
            const softDeleteSuccessfull =
              await this.edulevelRepository.softDelete(id, isActive, manager);
            if (!softDeleteSuccessfull) {
              throw new SomethinWentWrongException(
                'EduLevel soft delete failed',
              );
            }
