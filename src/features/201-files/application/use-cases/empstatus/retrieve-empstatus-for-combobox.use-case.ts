import { EmpStatusRepository } from '@features/201-files/domain/repositories/empstatus.repository';
import { Inject, Injectable } from '@nestjs/common';
import { CONSTANTS_REPOSITORY_TOKENS } from '@shared/constants/tokens.constants';

@Injectable()
export class RetrieveEmpStatusForComboboxUseCase {
  constructor(
    @Inject(CONSTANTS_REPOSITORY_TOKENS.EMPSTATUS)
    private readonly empstatusRepository: EmpStatusRepository,
  ) {}

  async execute(): Promise<{ value: string; label: string }[]> {
    const empstatuss = await this.empstatusRepository.retrieveForCombobox();

    return empstatuss.map((val: { desc1: string }) => ({
      value: val.desc1,
      label: val.desc1
        ? val.desc1.charAt(0).toUpperCase() + val.desc1.slice(1).toLowerCase()
        : '',
    }));
  }
}
