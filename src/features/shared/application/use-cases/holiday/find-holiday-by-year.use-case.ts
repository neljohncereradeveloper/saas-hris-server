...
@Injectable()
export class FindHolidayByYearUseCase {
  constructor(
  ...
  async execute(
    ...
    return this.transactionHelper.executeTransaction(
      CONSTANTS_LOG_ACTION.FIND_HOLIDAY_BY_YEAR,
      async (manager) => {
        const errorHandlerOptions = this.errorHandler.createOptions(
          ...
          { year },
          requestInfo,
          `Found holidays for year: ${year}`,
          `Failed to find holidays for year: ${year}`,
        );

        return this.errorHandler.executeWithErrorHandling(
          async () => {
            return await this.holidayRepository.findByYear(year, manager);
          },
          errorHandlerOptions,
          manager,
        );
