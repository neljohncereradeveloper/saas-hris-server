...
describe('FindCitizenshipPaginatedListUseCase', () => {
  let useCase: FindCitizenshipPaginatedListUseCase;
  let citizenshipRepository: jest.Mocked<CitizenShipRepository>;

  const mockCitizenships = [
    new CitizenShip({
      id: 1,
      desc1: 'Filipino',
      isActive: true,
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<FindCitizenshipPaginatedListUseCase>(
      FindCitizenshipPaginatedListUseCase,
    );
  ...
  describe('execute', () => {
    it('should return paginated list of citizenships', async () => {
      // Arrange
      const term = 'test';
      const page = 1;
      const limit = 10;
      citizenshipRepository.findPaginatedList.mockResolvedValue(
        mockPaginatedResult,
      );
      ...
      // Assert
      expect(citizenshipRepository.findPaginatedList).toHaveBeenCalledWith(
        term,
        page,
        limit,
      );
    ...
    it('should handle different page and limit values', async () => {
      // Arrange
      const term = 'citizenship';
      const page = 2;
      const limit = 5;
      const customResult = {
        data: mockCitizenships.slice(0, 2),
        meta: {
          page: 2,
          limit: 5,
          totalRecords: 7,
      ...
      citizenshipRepository.findPaginatedList.mockResolvedValue(customResult);
    ...
    it('should handle empty results', async () => {
      ...
      citizenshipRepository.findPaginatedList.mockResolvedValue(emptyResult);
    ...
    it('should handle repository errors', async () => {
      ...
      citizenshipRepository.findPaginatedList.mockRejectedValue(mockError);
